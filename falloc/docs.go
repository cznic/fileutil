...
Allocation/addressing granulation == atom == 16 bytes.
Block == n atoms.
File == n blocks.
...
Block first byte (Block type tag)
    ...
    0xFF
        ...
        +------++---------++---------++------+
        Link to the previous free block (atom addressed) is p6...p0, next dtto in n6...n0.
        Doubly linked lists of "compatible" free blocks allows for free space reclaiming and merging.
        "Compatible" == of size at least some K. Heads of all such lists are organized per K or intervals of Ks elsewhere.

    0xFE
        Free block, size == s6...s0 atoms.
        +------++---------++---------++---------++--
        |  +0  ||  1...7  || 8...14  || 15...21 || 22...16*size-1
        +------++---------++---------++---------++--
        | 0xFE || p6...p0 || n6...n0 || s6...s0 || ...
        +------++---------++---------++---------++--
        Prev and next links as in the 0xFF first byte case.
        End of this block - see "Block last byte": 0xFE bellow. Data between == undefined.
    ...
    0xFD
        ...
        +------++---------++-----------++------+
        Relocation link is r6..r0 == atom address. Relocations MUST NOT chain and MUST point to a "content" block,
        i.e. one with the first byte in 0x00...0xFC.
        Relocated block allows to permanently assign a handle/file pointer ("atom" address) to some content
        and resize the content anytime afterwards w/o having to update all the possible existing references
    ...
    0xFC
        ...
        +------++---------++--------------------++---------+---+
        This block type is used for content of length in N == 238...61680 bytes. N is encoded as a 2 byte
        unsigned integer n1..n0 in network byte order. Values bellow 238 are reserved, those content lengths
        are to be carried by the 0x00..0xFB block types.

        n in 0x00EE...0xF0F0 is used for content under the same rules as in the 0x01..0xED type.

            If the last byte of the content is not the last byte of an atom then the last byte of the block is 0x00.

            If the last byte of the content IS the last byte of an atom:

                If the last byte of content is in 0x00..0xFD then everything is OK.

                If the last byte of content is OxFE or OxFF then the escape via n > 0xF0F0 MUST be used
                AND the block's last byte is 0x00 or 0x01, meaning value 0xFE and 0xFF respectively.

		n in 0xF0F1...0xFFFF is like the escaped 0xEE..0xFB block.
			N == 13 + 16(n - 0xF0F1).

        Discussion of the padding and Z fields - see the 0x01..0xED block type.

    0xEE...0xFB
        Used escaped short block.
        +---++----------------------++---+
        | 0 ||        1...N-1       ||   |
        +---++----------------------++---+
        ...
        case Z == 0x00
            The last byte of content is 0xFE
        case Z == 0x01
            The last byte of content is 0xFF
    ...
    0x01...0xED
        ...
        +---++--------------------++---------+---+
        This block type is used for content of length in 1...237 bytes.
        The value of the "padding" field, if of non zero length, is undefined.
        ...
        If the last byte of content is the last byte of an atom AND the last byte
        of content is 0xFE or 0xFF then the short escape block type (0xEE...0xFB) MUST be used.
    ...
    0x00
        ...
        |  0   ||  1...14   ||  15  |
        +------++-----------++------+
        | 0x00 || undefined || 0x00 | // == used block, other "tail" values are reserved.
        +------++-----------++------+
        All of the rules for 0x01..0xED applies. Depicted only for it's different semantics
        (e.g. an allocated [existing] string but with length of zero).

Block last byte

    OxFF
        Free atom. Layout - see "Block firts byte": FF.

    OxFE
        Free block, size n atoms.
        Preceding 7 bytes == size (s6...s0) of the free block in atoms, network byte order
          --++---------++------+
            || -8...-2 ||  -1  |
          --++---------++------+
...
Header
    ...
    Special conditions apply:
        ...
        The header block and it's content MUST be like this:
...
Free lists table
    ...
    (both in network byte order), thus every item is 14 contiguous bytes. The item's address field is pointing to a free block.
    The size field determines the minimal size (in atoms) of free blocks on that list.

    The free list table is n above items, thus the content has 14n bytes. Note that the largest block
    content is 61680 bytes and as there are 14 bytes per table item, so the table is limited to at most 4405 entries.
    ...
    When freeing blocks, the block MUST be linked into an item list with the highest possible
    size field, which is less or equal to the number of atoms in the new free block.

    When freing a block, the block MUST be first merged with any adjacent free blocks (thus possibly creating a bigger
